...
impl FraudDetectionService {
    ...
    pub async fn process_event(&self, event: &Event) -> Option<FraudReport> {
        ...
        let data: crate::models::TradeCreatedData = serde_json::from_value(event.data.clone()).ok()?;
        ...
        let recent_events = self.db.get_events(&crate::models::EventQuery {
            limit: Some(100),
            offset: None,
        ...
        if let Some(res) = self.rules.check_linked_accounts(&recent_events, &data.seller, &data.buyer) {
            triggered_rules.push(res);
        }
    ...
    pub async fn process_confirmed_event(&self, event: &Event) -> Option<FraudReport> {
        ...
        let data: crate::models::TradeConfirmedData = serde_json::from_value(event.data.clone()).ok()?;
        let trade_id = data.trade_id;
        ...
        Some(FraudReport {
            ...
            risk_score: final_score,
            rules_triggered: triggered_rules,
            ml_result: MLResult { score: 0.0, contribution: "N/A".to_string() },
            status: (if final_score >= 80 { "high_risk" } else { "medium_risk" }).to_string(),
        })
    }
}
